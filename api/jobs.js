...
async function getJob(id) {
  if (durable) { const raw = await kv(['GET', `instagram:crawler:job:${id}`]); return raw ? JSON.parse(raw) : null; }
...
async function listJobs() {
  if (durable) { const ids = await kv(['LRANGE', 'instagram:crawler:jobs', '0', '19']); return (await Promise.all((ids || []).map(getJob))).filter(Boolean); }
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    if (id) { const job = await getJob(id); if (!job) return res.status(404).json({ error: 'Job not found' }); return res.status(200).json(job); }
